...
{{--        <p>{{$phone = Cookie::get('phone_number') }}</p>--}}
        <form id="verifyCodeForm" method="POST" action="{{ route('auth.verify-code') }}">
            ...
            <button id="smsTake" type="submit" class="mt-4 w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#3a86ff] hover:bg-[#3a86ff] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Подтвердить</button>
            <div>
                <div class="text-[10px] pt-2 text-center">Продолжая, вы соглашаетесь<a href="{{ asset('file/pass.doc') }}" download="{{ asset('file/pass.doc') }}" class="ml-1 text-blue-600">со
