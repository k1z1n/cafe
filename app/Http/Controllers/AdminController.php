...
class AdminController extends Controller
{
    ...
    public function addProduct(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'image_path' => 'required|image|mimes:jpeg,png,jpg,gif,webp|max:2048', // Обязательное поле для загрузки изображения
        ]);
        ...
        if ($request->hasFile('image_path')) {
            ...
            return redirect()->back()->with('error', 'Изображение не загружено')->withInput();
        }
    ...
    public function main()
    {
        ...
        $totalOrdersSum = NumberHumanizer::metricSuffix(Order::sum('total'));
        return view('admin.main', compact('productsCount','totalOrdersSum', 'usersCount', 'totalDeliveredOrders', 'totalVisits', 'weeklyVisits', 'dailyVisits'));
    }
